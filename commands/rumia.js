...
╰─────────────────────────────────────────────╯`
    ...
    // Message d'attente
    await client.sendMessage(remoteJid, { 
        text: `🧠 RUMIA AI réfléchit à ta question...\n\n📝 "${query.substring(0, 50)}${query.length > 50 ? '...' : ''}"`
    }, { quoted: message })
...
╰─────────────────────────────────────────────╯`
        ...
        console.error('Erreur Golden AI:', error)
...
export async function goldenClear(client, message) {
    const remoteJid = message.key.remoteJid
    const userId = message.key.participant || remoteJid
    ...
    await client.sendMessage(remoteJid, { 
        text: `🧹 Historique effacé !\n\nTes conversations avec Golden AI ont été réinitialisées.`
    }, { quoted: message })
}