...
text += "║ ${medal} @${user.name}${adminBadge} → ${user.rank.name}\n"
})
...
╚═══🌟═══🌟═════╝`
        ...
        const rankMessage = `╔═══𝐑𝐀𝐍𝐆📊═══╗
║${topDecor}${topDecor}${topDecor}        ${isSelf ? '📊 𝐌𝐎𝐍 𝐑𝐀𝐍𝐆 𝐋𝐄́𝐆𝐄𝐍𝐃𝐀𝐈𝐑𝐄' : '👤 𝐑𝐀𝐍𝐆 𝐃𝐔 𝐌𝐄𝐌𝐁𝐑𝐄'}        ${topDecor}${topDecor}${topDecor}║
╠═══════════╣

║ 👤 @${targetName}${isTargetAdmin ? ' 👑' : ''}
║ ${rankInfo.emoji} Niv.${rankInfo.level} : ${rankInfo.name}
║ 📜 ${rankInfo.title}

║ 📊 ${progressBar} ${rankInfo.progress}%
║ 💬 ${userMessages} msg
║ 🎯 ${rankInfo.nextLevelName}
