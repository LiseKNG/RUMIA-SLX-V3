...
import fs from 'fs';
import { handleDeletedMessage, handleStatusReply } from '../commands/antidelete.js'; // ✅ AJOUT
...
async function connectToWhatsapp(handleMessage) {
    ...
    // ✅ AJOUT DES EVENT LISTENERS ANTIDELETE ICI ✅
    sock.ev.on('messages.update', async (updates) => {
        for (const update of updates) {
            if (update.update?.message) {
                // Message modifié ou supprimé
                console.log('Message update détecté:', update)
            }
        }
    })

    // Pour les statuts supprimés
    sock.ev.on('messages.delete', async (deleted) => {
        const keys = deleted.keys
        for (const key of keys) {
            const chatJid = key.remoteJid
            const ownerJid = sock.user.id.split(':')[0] + '@s.whatsapp.net'
            await handleDeletedMessage(sock, key, chatJid, ownerJid)
        }
    })
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '237657989797@s.whatsapp.net';
...
┖═┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉═┙
├▹ I AM RUMIA-XMD-V3
╰▶︎ *Thank you for connecting to the bot💞*
╭═≼ 👤CREATOR👤 ≽⊶
║
╰▶︎ MAKER: BY 🌹ASZEL🌹⊣≽
...
╭═≼ 👑ADMIN👑  ≽⊶
║
╰▶︎ CONTACT: 237699239593⊣≽`;

                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by RUMIA-XMD-V3',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 237698303919;

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '237698303919' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'RUMIABOT');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['237657989797@s.whatsapp.net'],
